...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for(;start < end; start+=PGSIZE){
		...
		if(page_insert(e->env_pgdir, p, start, PTE_W | PTE_U) != 0){
			panic("Failed to map!");
		}
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	lcr3(PADDR(e->env_pgdir));		//work in user address space
	...
	lcr3(PADDR(kern_pgdir));		//return to kernel address space
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	if(env_alloc(&e, 0) != 0){
		panic("failed to create new environment");
	}
	load_icode(e, binary);
	e->env_type = type;
...
env_run(struct Env *e)
{
	...
	panic("env_run did not work successfully");
}
