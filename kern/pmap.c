...
boot_alloc(uint32_t n)
{
	...
	if(n == 0){																							//if we are not allocating bytes then just return nextfree
		return nextfree;
	}else{																											//otherwise: determine if there is sufficient space, then return the pointer to the head of newly allocated space
		result = nextfree;
		nextfree = ROUNDUP(nextfree+n, PGSIZE);										// and move nextfree to the end of the newly allocated space
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref !=0){
		panic("Processes are still utilizing this page\n");
	}
	if(pp->pp_link != NULL){
		panic("Page is free\n");
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	uintptr_t *ptr;
	for(size_t i = 0; i < size; i+= PGSIZE){					//increments space by page size
		ptr = pgdir_walk(pgdir, (void *)(va + i), 1);
		if(ptr == NULL){
			panic("Failed to map\n");
		} else{
			*ptr = (pa + i) | perm | PTE_P;
...
{
	// Fill this function in
	pte_t *ptr = pgdir_walk(pgdir, va, 1);
	if(ptr == NULL){
...
{
	// Fill this function in
	uintptr_t *ptr = pgdir_walk(pgdir, va, 0);


	if(ptr == NULL || (*ptr & PTE_P) == 0){			//check for page table and page
		return NULL;
	} else{
		if(pte_store != 0){
			*pte_store = ptr;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if(base +size > MMIOLIM){
		panic("Exceed mmio_map_region!");
	}
	boot_map_region(kern_pgdir, base, size, pa, PTE_W | PTE_PCD | PTE_PWT);

	uintptr_t rsbase = base;
	base += size;
	return (void *)rsbase;
