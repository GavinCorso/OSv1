...
sys_exofork(void)
{
	...
	struct Env *new_Env;
	int pass = env_alloc(&new_Env, curenv->env_id);
	if(pass < 0){
		return pass;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (srcva < (void *)UTOP) {
		...
		if(!page) {
			warn("sys_ipc_try_send: page not mapped");
			return -E_INVAL; //error 5
		}

		if((perm & PTE_W) && !(*pte & PTE_W)) {
			warn("sys_ipc_try_send: page not mapped");
			return -E_INVAL; //error 6
		}
		...
		if(!(perm & PTE_U) || !(perm & PTE_P) || (perm & ~PTE_SYSCALL)) {
			warn("sys_ipc_try_send: bad perms");
			return -E_INVAL; //error 3
		}

		if(dstenv->env_ipc_dstva < (void *) UTOP) {

			if(page_insert(dstenv->env_pgdir, page, dstenv->env_ipc_dstva, perm)) {
				warn("sys_ipc_try_send: page insert failed");
				return r; //error 7, 4
			}
		}
	...
	// things which must be done for every successful IPC
	dstenv->env_ipc_recving = 0;
	dstenv->env_ipc_from = curenv->env_id;
	dstenv->env_ipc_value = value;
	dstenv->env_status = ENV_RUNNABLE;
	dstenv->env_tf.tf_regs.reg_eax = 0; //set return value of dstenv to 0.
