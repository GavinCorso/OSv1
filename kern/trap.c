...
trap_init_percpu(void)
{
	...
	uint8_t cpuIdx = thiscpu->cpu_id;

	this_ts->ts_esp0 = KSTACKTOP - cpuIdx * (KSTKSIZE + KSTKGAP);
	this_ts->ts_ss0 = GD_KD;
	this_ts->ts_iomb = sizeof(struct Taskstate);

	gdt[(GD_TSS0 >> 3) + cpuIdx] = SEG16(STS_T32A,
																				(uint32_t) (this_ts),
																				sizeof(struct Taskstate)-1, 0);

	gdt[(GD_TSS0 >> 3) + cpuIdx].sd_s = 0;


	ltr(GD_TSS0 + (cpuIdx << 3));
	lidt(&idt_pd);
